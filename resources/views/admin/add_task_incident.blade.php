...
<?php //dd($data); ?>
 ...
 <div class="section" > 
    ...
    <section class="content-header">
      <h3>@if(@$request->id){{'Assign Incident to Task '}} @else {{'Add'}} @endif </h3>
      <ol class="breadcrumb">
        <li><a href="{{route('admin-dashboard')}}"><i class="fa fa-dashboard"></i> {{_i('Home')}}</a></li>
        <li class="active">@if(@$request->id){{'Assign Incident to Task'}} @else {{'Add'}} @endif </li>
      </ol>
    </section>
      ...
      <div class="row">
        <div class="col-md-10">
          ...
          <div class="box box-info">
            ...
            <form class="" id="add-files-form" action="@if(@$request->id) {{route('admin-incidentAndTaskMapped',['id'=>@$request->id])}} @else  {{route('admin-incidentAndTaskMapped')}} @endif" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
              <div class="box-body">
              ...
              <div class="form-group row @if($errors->first('title')) {{' has-error has-feedback'}} @endif ">
                  ...
                  <div class="col-sm-7">   
                    <select class="form-control incidentid"  name="incidentid" id='incidentid'>
                      <option value="">--Select Incident</option>
                      @if( !empty($incidentListArray) )
                        @foreach($incidentListArray as $key=>$value)
                          <option <?php if( !empty($data) && ($data->incident_id == $key) ) { echo 'selected = selected';} ?> value="{{ $key}}">{{ $value}}</option>
                        @endforeach  
                      @endif
                    </select>
