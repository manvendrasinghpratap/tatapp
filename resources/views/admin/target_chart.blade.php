...
@section('content')
<?php $linkedtype='case'; //dd($data['caseList']);?> 
<!-- START Datatable -->
...
<div class="clearfix"></div>
 <div class="section" > 
      <div class="container">
        ...
        <div class="paddingbottom10px"><h3>@if(@$request->id){{'Target Chart '}} @else {{'Target Chart'}} @endif </h3></div>
      <ol class="breadcrumb">
        <li><a href="{{route('admin-dashboard')}}"><i class="fa fa-tachometer-alt"></i> {{ ('Home')}}</a></li>
        <li class="active">@if(@$request->id){{'Target Chart'}} @else {{'Target Chart'}} @endif </li>
      </ol>
    </section>
</div>
