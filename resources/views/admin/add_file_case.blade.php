...
<?php //dd($data); ?>
 ...
 <div class="section" > 
    ...
    <section class="content-header">
      <h3>@if(@$request->id){{'Link Files to Cases'}} @else {{'Add'}} @endif </h3>
      <ol class="breadcrumb">
        <li><a href="{{route('admin-dashboard')}}"><i class="fa fa-dashboard"></i> {{_i('Home')}}</a></li>
        <li class="active">@if(@$request->id){{'Link Files to Cases'}} @else {{'Add'}} @endif </li>
      </ol>
    </section>
      ...
      <div class="row">
        <div class="col-md-10">
          ...
          <div class="box box-info">
            ...
            <form class="" id="add-files-form" action="@if(@$request->id) {{route('admin-caseAndFilesMapped',['id'=>@$request->id])}} @else  {{route('admin-caseAndFilesMapped')}} @endif" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
              <div class="box-body">
                    ...
                    <?php  $casesfilesArray  = array();
                                if(!empty( $data->casesfiles) ){
                                    ...
                                    foreach ($casesName as $key => $value) {
                                        $casesfilesArray[] = $value->cases->id;
                                    }
                                                                      
                                }
                               ?>
                    <input type="hidden" name="existingcaseId"  value="<?php echo implode(',', $casesfilesArray);?>">           
                    <select class="form-control cases" multiple="true" name="caseid[]" id='cases'>
                      <option value="">--Select Case</option>
                      @if( !empty($caseListArray) )
                        @foreach($caseListArray as $key=>$value)
                          <option <?php if( !empty($casesfilesArray) && in_array($key,$casesfilesArray) ) { echo 'selected = selected';} ?> value="{{ $key}}">{{ $value}}</option>
                        @endforeach  
                      @endif
                    </select>
