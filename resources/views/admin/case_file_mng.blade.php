                        ...
                        <form id="user-mng" method="get" action="{{route('admin-caseList')}}">
                            ...
                            <div class="row clearfix">
                                ...
                                <div class="col-sm-10">
                                    ...
                                    <?php } ?>
                                    <div class="col-sm-3 pull-right">
                                    <a href="{{route('admin-casefileist')}}" class="btn btn-block btn-warning">Case Files</a>
                                    </div>
                                    <div class="col-sm-3 pull-right">
                                    <a href="{{route('admin-incidentfilelist')}}" class="btn btn-block btn-warning">Incident Files</a>
                                    </div>
                                    </div>                  
                            </div>
                        </form> 
                    ...
                    <div class="box-body table-responsive no-padding">
                        <table class="table table-hover" id="printTable">
                            <tr>
                                ...
                                <th >{{_i('Case Name')}}</th>
                                <th >{{_i('Title')}}</th>
                                <th width="25%" style=" word-break: break-all; ">{{_i('Description')}}</th>
                                <th width="25%">{{_i('Image')}}</th>
                            ...
                            <tr>
                               <!-- <td scope="row">{{$k}}</td>-->
                               <td>{{@$row->cases->title}}</td>
                               <td>{{@$row->title}}</td>
                               <td> <div style="word-wrap: break-word; width: 90%;" > {{@$row->description}}</div></td>
                                <td><?php if(!empty($row->profile_pic))
                                    { 
                                        $ext = explode('.',$row->profile_pic); 
                                        $fileExt = end($ext);
                                        if(in_array($fileExt,$extArray)){
                                            ?><img src="{{get_image_url(@$row->profile_pic,'files')}}" style="height:100px"><?php 
                                        }else{
                                           ?><a href="/files/">Download file</a><?php 
                                        } 
                                  }   
                                  ?>   </td>
