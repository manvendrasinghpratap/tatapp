...
<!-- Trigger the modal with a button -->
        <?php  $existingincidentIds = array();
            if(!empty( $data->reportCaseId  )){
              foreach ($data->reportCaseId as $key => $value) {
        ...
        <div class="modal modal show" tabindex="1" role="dialog" id="myModal">
            <div class="modal-dialog" role="document" style="width: 85%; background-color: white;" >
                <div class="box-body table-responsive no-padding" style="padding:30px;">
                    <input type="hidden" name="existingincidentIds" id="existingincidentIds" value="{{implode(',',$existingincidentIds)}}">
                    <input type="hidden" name="caseId" id="caseId" value="{{$data->id}}">
...
$(document).ready(function () { 
    ...
    $('.add-incidentid').on('click', function(){
       ...
       /* $('#incidentids').val(arr);
        ...
        $('#modalBt').hide();
        $('.error').hide();*/
        var existingincidentIds = $('#existingincidentIds').val();
        var caseId              = $('#caseId').val();
        savecaseincidentdata(arr,existingincidentIds,caseId,'{{route('admin-ajaxLinkIncidentAndCaseId')}}');
        }
