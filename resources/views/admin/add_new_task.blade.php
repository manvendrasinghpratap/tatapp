...
<div class="clearfix"></div>
 <div class="section" >
      ...
      <div class="row">
        <div class="col-md-12">
          ...
          <div class="box box-info">
            ...
            <!-- /.box-header -->
            <!-- form start -->
           <?php $existingcaseid = array();$existingcaseName = array();
            if(!empty( $data['taskDetailsArray']->casetasklist  )){
              foreach ($data['taskDetailsArray']->casetasklist as $key => $value) {
                $existingcaseid[] = $value->case_id;
                $existingcaseName[] = '<span class="red"> '. $value->case->title.'</span>';
              }
            } 
           ?>
           
           <?php $existingincidentIds = array(); $existingincidentname = array();
            if(!empty( $data['taskDetailsArray']->incidenttasks  )){
              foreach ($data['taskDetailsArray']->incidenttasks as $key => $value) {
                $existingincidentIds[] = $value->incident_id;
                $existingincidentname[] = '<span class="red"> '. $value->incident->title.'</span>';
              }
            } 
           ?>

            <form class="" id="add-task-form" action=" {{route('admin-updateincidentTask')}} " method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
              <div class="box-body">
              <input type="hidden" name="id" value="{{old('id',@$data['taskDetailsArray']->id)}}">
              <div class="form-group row @if($errors->first('title')) {{' has-error has-feedback'}} @endif ">
                ...
                <div class="form-group row @if($errors->first('task_assigned')) {{' has-error has-feedback'}} @endif " style="display: none;">
                    ...
                    <div class="col-sm-7"> 
                        ...
                        <?php  $selectedClass = (isset($data['taskDetailsArray']->zone) && $data['taskDetailsArray']->zone==$row)?'selected':'';  ?>  
                ...
                <input type="hidden" name="caseids" id="caseids" value="">
                <input type="hidden" name="incidentids" id="incidentids" >
              <!-- /.box-body -->
              <div class="box-footer">
                <input type="hidden" name="task_id" id="task_id" class="taskClass" value="{{@$data['taskDetailsArray']->id}}">
                <a href="{{route('admin-task-list')}}" class="btn btn-default">Cancel</a>
                @if($request->mode=='edit') <input type="submit" id="add-button" value="Submit" class="btn btn-info" >  @endif
              </div>
              <!-- /.box-footer -->
              
           <input type="hidden" name="existingincidentid" value="{{ implode(',',$existingincidentIds)}}">
           <input type="hidden" name="existingcaseid" value="{{ implode(',',$existingcaseid)}}">
  ...
  <script type="text/javascript">
    $("#add-task-form :input").prop("disabled", true);
  </script>
  @endif
  @endsection
