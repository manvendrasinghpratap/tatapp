                    ...
                    <div class="box-body table-responsive no-padding">
                         <table class="table table-hover" id="printTable">
                            ...
                            <tr>
                               ...
                               <td> @if( !empty( explode('.',$row->newfilename)[1] ) ) {{ explode('.',$row->newfilename)[1] }}  @endif </td>
                                <td>{{date("F j, Y", strtotime($row->created_at))}}</td>
                                @if(Session::get('user_role_id')<=4)
                                <td>                                     
                                    <a href="{{route('admin-viewFile',['id'=>$row->id])}}" class="btn btn-info btn-xs action-btn" title ="View Detail"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></a>
                                    <a href="{{route('admin-editFile',['id'=>$row->id])}}" class="btn btn-info btn-xs action-btn" title ="Edit Package"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a>
                                    <a href="{{route('admin-caseAndFilesMapped',['id'=>$row->id])}}" class="btn btn-info btn-xs action-btn" title ="Map with Cases"><span class="glyphicon glyphicon-adjust" aria-hidden="true"></span></a>
                                    <a href="{{route('admin-incidentAndFilesMapped',['id'=>$row->id])}}" class="btn btn-info btn-xs action-btn" title ="Map with Incident"><span class="glyphicon glyphicon-ban-circle" aria-hidden="true"></span></a>
                                    <a href="{{route('admin-deleteFile',['id'=>$row->id])}}" class="btn btn-info btn-xs action-btn deleteFile" title ="Delete"  data-link1="" data-link2="{{$row->id}}"><span class="glyphicon glyphicon-trash" aria-hidden="true" ></span></a>   
                                </td>
                                @endif
                            </tr>
                            <?php $k++; ?>     
