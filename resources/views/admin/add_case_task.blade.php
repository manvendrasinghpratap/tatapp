...
<?php //dd($data); ?>
 ...
 <div class="section" > 
    ...
    <section class="content-header">
      <h3>@if(@$request->id){{'Link Case to Tasks '}} @else {{'Add'}} @endif </h3>
      <ol class="breadcrumb">
        <li><a href="{{route('admin-dashboard')}}"><i class="fa fa-dashboard"></i> {{_i('Home')}}</a></li>
        <li class="active">@if(@$request->id){{'Link Case to Tasks'}} @else {{'Add'}} @endif </li>
      </ol>
    </section>
      ...
      <div class="row">
        <div class="col-md-10">
          ...
          <div class="box box-info">
            ...
            <form class="" id="add-files-form" action="@if(@$request->id) {{route('admin-caselinkwithtask',['id'=>@$request->id])}} @else  {{route('admin-caselinkwithtask')}} @endif" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
              <div class="box-body">
              ...
              <div class="form-group row @if($errors->first('title')) {{' has-error has-feedback'}} @endif ">
                  <label for="inputError" class="col-sm-3 control-label"> Tasks {{redstar()}}</label>
                  <div class="col-sm-7">
                  <?php  
                  ...
                  <input type="hidden" name="existingtaskId"  value="<?php echo implode(',', $existingTaskId);?>"> 
                    <select class="form-control incidentid"  name="taskId[]" id='taskId' multiple="true">
                      <option value="">--Select Task</option>
                      @if( !empty($taskListArray) )
                        @foreach($taskListArray as $key=>$value)
                          <option <?php if( !empty($existingTaskId) && in_array($value->id,$existingTaskId) ) { echo 'selected = selected';} ?> value="{{ $value->id}}">{{ $value->title}}</option>
                        @endforeach  
                      @endif
                    </select>
